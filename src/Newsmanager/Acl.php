...
class Acl extends DBObject implements AclInterface
{
    ...
    public function hasPermission(
        ...
        if($this->id_user == null) {
            ...
            $stmt->close();
            //Noone has readpermission explicitly set,
            //hence everyone has read permission
            if($result===false) { 
                return true;
            } else {
            	//Some people have read permissions set,
            	//hence we must ensure the specific user has read permission
                $this->id_user = $user->id; 
            }
        }
        $stmt = $this->getSearchStmt();
        ...
        $result = $result->fetchArray();
        $stmt->close();
        //User does not have this permission set and this permission is not read
        if($result === false) {
            return false;
        } else {
            return true;
